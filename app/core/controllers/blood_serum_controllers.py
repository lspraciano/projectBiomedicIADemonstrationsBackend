...
async def get_blood_serum_predictions(
        blood_serum_yolo_model: YOLO,
        image: Image,
) -> Optional[Tuple[Image, Dict]]:
    """
    Performs predictions of blood serum images using a YOLO model.

    **Parameters**

    * **blood_serum_yolo_model** (YOLO): A YOLO model trained to detect objects in blood serum images.
    * **image** (Image): The blood serum image to be processed.
    ...
    **Returns**
    ...
    predict_result_list: List[Results] = blood_serum_yolo_model.predict(
        source=image,
        verbose=False,
        conf=0.5
    )
    ...
    result_dict: Dict = convert_result_to_dict(
        result=predict_result,
        classes=blood_serum_yolo_model.names
    )
