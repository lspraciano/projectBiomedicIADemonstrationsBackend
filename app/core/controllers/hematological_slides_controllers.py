...
async def get_hematological_slides_predictions(
        wbc_yolo_model: YOLO,
        image: Image,
) -> Optional[Tuple[Image, Dict]]:
    """
    Performs detections of white blood cells in a hematological slide image.

    **Parameters**

    * **wbc_yolo_model** (YOLO): A YOLO model trained to detect white blood cells.
    * **image** (Image): The hematological slide image to be processed.
    ...
    **Returns**
    ...
    predict_result_list: List[Results] = wbc_yolo_model.predict(
        source=image,
        verbose=False,
        conf=0.5
    )
